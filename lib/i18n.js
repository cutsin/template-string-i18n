...
const dump = () => {
  locales.forEach(lang => {
    // simple merge
    let src = read(Cache[lang].dir)
    for(k in src) {
      let v_src = src[k]
      let v_new = Cache[lang].data[k]
      if (v_new) src[k] = v_new
    }
    write(Cache[lang].dir, src, true)
  })
}
...
locales.forEach(lang => {
  ...
  !fs.existsSync(dir) ? write(dir) : Cache[lang].data = read(dir)
})
